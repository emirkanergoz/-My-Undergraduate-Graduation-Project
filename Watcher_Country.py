...
encokizlenenulke = df["watcher_country"].value_counts().idxmax()
df["watcher_country"] = df["watcher_country"].fillna(encokizlenenulke)

attacker_countries = df["watcher_country"].unique().tolist()
...
df["watcher_continent"] = df["watcher_country"].map(country_to_continent)


# continent sütunundaki boş değerlere karşılık gelen 'attacker_country' değerlerini gösterin
missing_countries_codes = (df[df["watcher_continent"].isna()]["watcher_country"]).unique().tolist()

print(df["attacker_as_name"].unique())

watcher_continent = pd.get_dummies(df["watcher_continent"], prefix="continent_watcher",dtype=int)
watcher_continent.to_parquet("watcher_continent.parquet")
df = pd.concat([df, watcher_continent], axis=1)
df.drop(["watcher_country","watcher_continent"], axis=1,inplace=True)
print(df.head())